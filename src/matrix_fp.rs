...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum LinearSolveFp {
    Consistent {
        solution: Vec<u64>,
        free_columns: Vec<usize>,
        rank: usize,
        active_nonzero_columns: Vec<usize>,
    },
    Inconsistent {
        rank: usize,
    },
...
pub(crate) fn solve_linear_system_fp(
    matrix: &[Vec<u64>],
    rhs: &[u64],
    ...
    let augmented = matrix
        ...
        .collect::<Vec<_>>();
    let reduction = row_reduce_matrix_fp(&augmented, modulus);
    let rank = reduction
        .pivot_columns
        .iter()
        .filter(|col| **col < cols)
        .count();
    if reduction.pivot_columns.contains(&cols) {
        return LinearSolveFp::Inconsistent { rank };
    }

    let mut solution = vec![0; cols];
    for (row, pivot_col) in reduction.pivot_columns.iter().copied().enumerate() {
        if pivot_col < cols {
            solution[pivot_col] = reduction.rref[row][cols];
        }
    }
    let free_columns = (0..cols)
        .filter(|col| !reduction.pivot_columns.contains(col))
        .collect::<Vec<_>>();
    let active_nonzero_columns = active_nonzero_support(&solution);

    LinearSolveFp::Consistent {
        solution,
        free_columns,
        rank,
        active_nonzero_columns,
    }
}
...
pub(crate) fn active_nonzero_support(vector: &[u64]) -> Vec<usize> {
    vector
        .iter()
        .enumerate()
...
mod tests {
    ...
    #[test]
    fn finite_field_solver_reports_rank_solution_and_active_support() {
        let modulus = PrimeModulus::new(7).unwrap();
        let matrix = vec![vec![1, 2, 0], vec![0, 1, 1]];
        let rhs = vec![5, 4];

        let solve = solve_linear_system_fp(&matrix, &rhs, modulus);

        let LinearSolveFp::Consistent {
            solution,
            free_columns,
            rank,
            active_nonzero_columns,
            ...
            panic!("system should be consistent");
        };
        assert_eq!(rank, 2);
        assert_eq!(free_columns, vec![2]);
        assert_eq!(matrix_vector_mul_fp(&matrix, &solution, modulus), rhs);
        assert_eq!(active_nonzero_columns, active_nonzero_support(&solution));
        assert!(!active_nonzero_columns.is_empty());
    }
}
